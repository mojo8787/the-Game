...
func handle_scanning(delta: float):
	...
	scan_beam.points[1] = beam_direction * 80  # Updated for new beam length
...
func change_state(new_state: DroneState):
	...
	print("Animated Drone state changed to: ", DroneState.keys()[new_state])
...
func start_thruster_effects():
	if thruster_particles:
		...
		rotation_tween.tween_property(animated_sprite, "rotation", deg_to_rad(3), 2.5)
		rotation_tween.tween_property(animated_sprite, "rotation", deg_to_rad(-3), 2.5)
...
func _on_detection_area_body_entered(body: Node) -> void:
	if body.has_method("_physics_process") and body != self:
		print("Player detected by animated drone!")
...
func _on_detection_area_body_exited(body: Node) -> void:
	if body == player_reference:
		print("Player lost by animated drone!")
